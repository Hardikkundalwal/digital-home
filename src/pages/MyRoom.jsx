...
import TriviaPanel from '../components/room/panels/TriviaPanel';
...
import { Bed, Pencil, Timer, Settings, HelpCircle, ArrowLeft, Gamepad2, User, X } from 'lucide-react';
...
const PANEL_META = {
  ...
  bed: { title: <><Bed size={18} strokeWidth={1.5} /> Wind down</>, roomId: null },
  trivia: { title: <><Gamepad2 size={18} strokeWidth={1.5} /> Trivia</>, roomId: null },
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <div className="scene-bar">
        ...
        <h1 style={{ flex: 1 }}><Bed size={16} strokeWidth={1.5} /> Room</h1>
        <MiniPlayer />
      ...
      <Suspense fallback={<div className="room-loading">Loading room…</div>}>
        <Room3D roomId={ROOM} wallColor="#ede8e2" floorColor="#c8b898" ceilingColor="#f0e8dd">
          <MyRoomScene onFurnitureClick={handleFurnitureClick} roomCode={ROOM} />
        </Room3D>
      </Suspense>
      <RoomSheet open={!!active || showHelp} onClose={() => { handleClose(); setShowHelp(false); }} title={showHelp ? <><HelpCircle size={18} strokeWidth={1.5} /> Help</> : (active ? PANEL_META[active]?.title : '')}>
        ...
        {!showHelp && active === 'bed' && <BedPanel />}
        {!showHelp && active === 'trivia' && <TriviaPanel />}
